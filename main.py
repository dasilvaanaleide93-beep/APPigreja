...
def carregar(a):
    if os.path.exists(a):
        try:
            with open(a,"r",encoding="utf-8") as f: return json.load(f)
        except: return []
    return []

def salvar(a,d):
    with open(a,"w",encoding="utf-8") as f: json.dump(d,f,ensure_ascii=False,indent=2)
...
HTML = """
<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Paroquia Santa Terezinha</title>
<style>
...
.banner{width:100%;height:300px;object-fit:cover;object-position:top center;display:block;background:#fdf8ef}
...
<div id="home"><div class="card-top">
...
<div class="grid">
<div class="item" onclick="abrir('doacoes')"><div style="font-size:44px">💝</div><p>Doacoes</p></div>
<div class="item" onclick="abrir('oracao')"><div style="font-size:44px">🙏</div><p>Pedido de<br>Oracao</p></div>
<div class="item" onclick="abrir('liturgia')"><div style="font-size:44px">📖</div><p>Liturgia<br>Diaria</p></div>
</div>
...
<div class="bloco"><p style="font-weight:bold;margin:0 0 10px 0">Voluntarios</p><div class="item" style="width:110px" onclick="abrir('vol')"><div style="font-size:42px">👥</div><p>Seja um</p></div></div>
</div></div>
<div id="oracao" class="pag"><button class="voltar" onclick="abrir('home')">← Voltar</button><h2>Pedido de Oracao</h2><input id="nomeO" placeholder="Seu nome"><textarea id="pedO" placeholder="Seu pedido..."></textarea><button class="btn" onclick="enviarO()">Enviar</button><div id="listaO"></div></div>
<div id="doacoes" class="pag"><button class="voltar" onclick="abrir('home')">← Voltar</button><h2>Doacoes</h2><div style="background:#e8f5e9;padding:20px;border-radius:12px;text-align:center">PIX da Paroquia<br>Barra do Garcas - MT</div></div>
<div id="liturgia" class="pag"><button class="voltar" onclick="abrir('home')">← Voltar</button><h2>Liturgia Diaria</h2><p>Que Santa Terezinha interceda por nos! 🌹</p></div>
...
<div id="vol" class="pag"><button class="voltar" onclick="abrir('home')">← Voltar</button><h2>Voluntarios</h2><input id="nomeV" placeholder="Seu nome"><input id="zapV" placeholder="WhatsApp"><textarea id="areaV" placeholder="Como ajudar?"></textarea><button class="btn" onclick="enviarV()">Quero ser Voluntario</button><div id="listaV"></div></div>
<script>
...
async function carregarO(){let r=await fetch('/oracao');let d=await r.json();let h='';d.oracoes.slice().reverse().forEach(p=>{h+=`<div class='card'><b>${p.nome||'Anonimo'}</b><br>${p.pedido||''}</div>`});document.getElementById('listaO').innerHTML=h;}
...
async function carregarV(){let r=await fetch('/voluntarios');let d=await r.json();let h='<h3>Voluntarios:</h3>';d.voluntarios.slice().reverse().forEach(p=>{h+=`<div class='card'><b>${p.nome||''}</b> - ${p.zap||''}<br>${p.area||''}</div>`});document.getElementById('listaV').innerHTML=h;}
