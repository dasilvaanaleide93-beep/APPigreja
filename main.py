...
.banner{width:100%;height:220px;display:block;object-fit:cover;object-position:center 20%;background:#fff}
.menu{display:flex;justify-content:space-around;background:#8B0000;padding:12px 0}
.menu button{background:none;border:none;color:white;font-size:22px}
.menu span{display:block;font-size:11px;margin-top:2px}
.pag{display:none;padding:20px}
.pag.ativa{display:block}
#home.ativa{display:block;text-align:center;padding:30px 10px}
.icones{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:20px}
.icone{background:white;border-radius:15px;padding:20px;box-shadow:0 2px 5px #ccc}
.icone b{display:block;font-size:40px}
...
<div class="menu">
<button onclick="abrir('home')">🏠<span>Início</span></button>
<button onclick="abrir('oracao')">🙏<span>Oração</span></button>
<button onclick="abrir('liturgia')">📖<span>Liturgia</span></button>
<button onclick="abrir('doacoes')">❤️<span>Doações</span></button>
<button onclick="abrir('mural')">💬<span>Mural</span></button>
</div>
...
<div id="liturgia" class="pag"><h3>Liturgia Diária</h3><p>Em breve...</p></div>
<div id="doacoes" class="pag"><h3>Doações</h3><p>Em breve...</p></div>
<div id="mural" class="pag"><h3>Mural de Recados</h3><p>Em breve...</p></div>
